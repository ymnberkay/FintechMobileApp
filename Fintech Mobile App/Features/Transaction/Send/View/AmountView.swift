...
struct AmountView: View {
    ...
    private var userProfileView: some View {
        VStack(spacing: 16) {
            // Avatar
            if let avatarImageName = viewModel.selectedReciever?.fullName {
            ...
            // User Info
            VStack(spacing: 4) {
                Text(viewModel.selectedReciever?.fullName ?? "unknown")
                    .font(Typography.headlineLargeSemibold)
                    .foregroundColor(ColorPalette.contentPrimary)
                
                Text(viewModel.selectedReciever?.email ?? "unknown")
    ...
    private var continueButtonView: some View {
        Button(action: {
            ...
            if viewModel.isValidAmount {
                coordinator.push(.accountSelect)
            }
        }) {
            HStack {
